...
## :desc
Команда `docker exec` используется для выполнения команды в уже запущенном контейнере. Это полезно для выполнения задач или диагностики внутри контейнера без необходимости перезапуска.
...
## :desc
Команда `docker exec -it [container] bash` позволяет войти в работающий контейнер и использовать указанную командную оболочку. В данном случае `bash`.
